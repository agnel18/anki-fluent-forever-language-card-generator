...
17. ~~**Latvian shallow per-word explanations + crash + adjective misclassification**~~ — **RESOLVED (May 2026).** Three bugs combined to ship `"Šis (pronoun): Šis (pronoun)"` stubs at beginner difficulty: (a) `lv_analyzer.py:255,268` called `response.text.strip()` without null-guarding — Gemini returned 200 OK with `.text = None` (safety filter / max-tokens), raising `AttributeError` that propagated to `batch_analyze_grammar`'s outer except. (b) The rule-based fallback in `lv_fallbacks._classify_word` emitted single-line POS stubs (`"{word} (pronoun)"`) and gated the `adjective_definite` branch behind `complexity != "beginner"` — so at beginner level, adjectives like `jaunais`, `labs` fell through to the `-s` masculine-noun rule. (c) The Latvian prompt asked for a one-word `meaning` field; German's prompt asks for a multi-sentence `individual_meaning` field. Same shallow-fallback pattern existed in Arabic, Spanish, Turkish (4 of 13 analyzers total). Fix: added `_extract_response_text` null-guard helper; rewrote prompt to request `individual_meaning` with worked examples and "DO NOT label adjectives/determiners as noun" rule; rewrote parser to preserve `individual_meaning` and format as `"{word} ({role}): {explanation}"` matching `de_response_parser.py:278-286`; rewrote `lv_fallbacks` with multi-clause templates, possessive determiner / demonstrative / personal pronoun lexicons, adjective check ahead of noun check at all complexities, and per-complexity role softening; strengthened `validate_explanation_quality` to penalize <20-char stubs and missing case/gender/agreement keywords on inflected words; enriched Arabic/Spanish/Turkish fallback templates similarly; added `is_fallback: True` flag + validator short-circuit to Spanish (mirrors German pattern at `de_validator.py:24`) so rich fallback content doesn't inflate the validator score. See "Rich-explanation contract" in Coding Conventions.
...
- **Rich-explanation contract** (cross-cutting rule for ALL analyzers — German is the reference): every per-word explanation displayed to the learner must be a multi-sentence string (≥30 chars) naming the word's syntactic role + morphology + language-specific feature, NOT a stub like `"{word} ({role})"`. Required components: (1) prompt asks for an `individual_meaning` field with 3-4 worked examples; (2) parser preserves `individual_meaning` and formats display as `f"{word} ({role}): {individual_meaning}"`; (3) fallback emits multi-clause templates (never POS-only stubs); (4) `_call_ai` null-guards `response.text.strip()` (Gemini can return 200 OK with `.text = None` on safety filter / max-tokens) — raise `RuntimeError` so the existing retry layer can swap models; (5) `validate_explanation_quality` penalizes <20-char bodies and missing case/gender/tense/agreement keywords on inflected words; (6) fallback dict carries `is_fallback: True` + validator short-circuits confidence to 0.3 when present (without this, rich fallback content inflates the validator score). Adjective definite/indefinite vs noun classification must run at **all** complexity levels — gating it behind `complexity != "beginner"` is the historic Latvian bug. Possessive determiners (Latvian `mans/tava`, Russian `мой/твой`) must be in a closed-class lexicon — suffix heuristics catch them as `-s` masculine nouns. Reference: German across the board (`de_*.py`); fixed in Latvian/Arabic/Spanish/Turkish in May 2026.
