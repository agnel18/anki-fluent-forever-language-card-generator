...
def render_complete_page():
    ...
    if "words_data" in st.session_state:
        for word_data in st.session_state.words_data:
            if "sentences" in word_data:
                for sentence_data in word_data["sentences"]:
                    if "validation_warning" in sentence_data and sentence_data["validation_warning"].get("issues"):
                        validation_warnings.append({
                            "word": word_data.get("word", "Unknown"),
                            "sentence": sentence_data.get("sentence", ""),
                            "issues": sentence_data["validation_warning"]["issues"]
                        })

    if validation_warnings:
        st.markdown("## ⚠️ Sentence Validation Warnings")
        st.markdown("Some sentences didn't meet quality standards. They were included anyway, but please review them:")
        st.markdown("")

        for warning in validation_warnings:
            with st.expander(f"⚠️ **{warning['word']}** - Issues: {', '.join(warning['issues'])}"):
                st.markdown(f"**Sentence:** {warning['sentence']}")
                st.markdown(f"**Issues:** {', '.join(warning['issues'])}")
                st.markdown("*This sentence was generated by AI but doesn't meet our quality standards. Please check if it's acceptable for your learning needs.*")

        st.markdown("")
        st.info("💡 **Tip:** These sentences are still included in your deck. If they're not suitable, you can edit or delete them in Anki after import.")
