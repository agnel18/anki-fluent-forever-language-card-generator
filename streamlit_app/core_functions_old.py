...
try:
    from .sentence_generator import generate_sentences_for_words, generate_ai_sentences
...
try:
    from .audio_generator import generate_audio_for_sentences, generate_audio_for_word
...
try:
    from .image_generator import generate_images_for_words
...
try:
    from .deck_exporter import export_deck_to_anki
...
try:
    from .generation_utils import (
        validate_inputs, prepare_output_directory, cleanup_temp_files,
        get_generation_stats, log_generation_event, validate_language_support
    )
...
__all__ = [
    # Sentence generation
    'generate_sentences_for_words', 'generate_ai_sentences',
    # Audio generation
    'generate_audio_for_sentences', 'generate_audio_for_word',
    # Image generation
    'generate_images_for_words',
    # Deck export
    'export_deck_to_anki',
    # Utilities
    'validate_inputs', 'prepare_output_directory', 'cleanup_temp_files',
...
# Import sentence generation functions (with fallback)
try:
    from sentence_generator import generate_sentences, generate_word_meaning, generate_ipa_hybrid
    logger.info("Using new sentence_generator module")
except ImportError as e:
    logger.warning(f"Could not import sentence_generator: {e}, using fallback functions")
    # Fallback functions will be defined later in the file

# Import audio generation functions (with fallback)
try:
    from audio_generator import generate_audio, _voice_for_language, get_available_voices
    logger.info("Using new audio_generator module")
except ImportError as e:
    logger.warning(f"Could not import audio_generator: {e}, using fallback functions")
    # Fallback functions will be defined later in the file

# Import image generation functions (with fallback)
try:
    from image_generator import generate_images_pixabay, generate_images_batch
    logger.info("Using new image_generator module")
except ImportError as e:
    logger.warning(f"Could not import image_generator: {e}, using fallback functions")
    # Fallback functions will be defined later in the file

# Import deck export functions (with fallback)
try:
    from deck_exporter import create_anki_tsv, create_zip_export, create_apkg_export, read_tsv_rows
    logger.info("Using new deck_exporter module")
except ImportError as e:
    logger.warning(f"Could not import deck_exporter: {e}, using fallback functions")
    # Fallback functions will be defined later in the file

# Import generation utilities (with fallback)
try:
    from generation_utils import estimate_api_costs, parse_csv_upload
    logger.info("Using new generation_utils module")
except ImportError as e:
    logger.warning(f"Could not import generation_utils: {e}, using fallback functions")
    # Fallback functions will be defined later in the file
