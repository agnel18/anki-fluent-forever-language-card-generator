...
class EsAnalyzer(BaseGrammarAnalyzer):
    ...
    def get_sentence_generation_prompt(self, word: str, language: str, num_sentences: int,
        ...
        """
        Get Spanish-specific sentence generation prompt to ensure proper response formatting.
        """
        ...
        # Custom prompt for Spanish to ensure proper formatting
        prompt = f"""You are a native-level expert linguist in Spanish.

Your task: Generate a complete learning package for the Spanish word "{word}" in ONE response.
...
Examples: gender agreement (masculine/feminine), number agreement, verb conjugations.
...
STEP 2: SENTENCES
===========================
Generate exactly {num_sentences} highly natural, idiomatic sentences in Spanish for the word "{word}".
...
- Use different verb tenses and moods if applicable
- Use different sentence types: declarative, interrogative, imperative
- Include appropriate gender and number agreement when applicable
- Use different regional variations where appropriate
{context_instruction}
...
- Use standard IPA symbols for Spanish pronunciation
...
SENTENCES:
1. [sentence 1 in Spanish]
2. [sentence 2 in Spanish]
3. [sentence 3 in Spanish]
4. [sentence 4 in Spanish]
