# languages/malayalam/domain/ml_response_parser.py
"""
Malayalam Response Parser - Domain Component

Parses AI responses with 5-level fallback JSON extraction.
Handles Malayalam-specific transformations for agglutinative morphology.
"""
...
from .ml_fallbacks import MlFallbacks

logger = logging.getLogger(__name__)


class MlResponseParser:
    """Parses AI responses for Malayalam grammar analysis."""

    def __init__(self, config: MlConfig, fallbacks: MlFallbacks):
        self.config = config
    ...
    def _transform_to_standard_format(self, data: Dict[str, Any], complexity: str, target_word: str = None) -> Dict[str, Any]:
        """Transform parsed data to standard format with Malayalam-specific processing."""
        ...
        for word_data in words:
            ...
            explanation = self._build_malayalam_explanation(word_data, standard_role)
    ...
    def _build_malayalam_explanation(self, word_data: Dict[str, Any], standard_role: str) -> str:
        """Build explanation with Malayalam-specific details."""
        individual_meaning = word_data.get('individual_meaning', '')
        if individual_meaning:
            # Append case info if available
            case = word_data.get('case', '')
            if case and case != 'null' and case != 'nominative':
                return f"{individual_meaning} [{case} case]"
